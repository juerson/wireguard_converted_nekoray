...
if __name__ == '__main__':
    ...
    if input_mtu.isdigit():
        base_str = update_base_info(conf_file=files[0], MTU=input_mtu)  # 调用函数
    else:
        base_str = update_base_info(conf_file=files[0])  # 调用函数
    endpoints = read_ip_endpoints(csv_file=files[1])
    output_file = 'output-links.txt'  # nekoray节点保存到这里
