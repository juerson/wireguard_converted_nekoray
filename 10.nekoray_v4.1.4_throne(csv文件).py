...
if __name__ == '__main__':
    ...
    for i, endpoint in enumerate(endpoints):
        ...
        throne_link = f'wg://{ip}:{port}?private_key={quote(prik)}&peer_public_key={quote(pubk)}&pre_shared_key=&reserved={rsvd}&persistent_keepalive=0&mtu={mtu}&use_system_interface=false&local_address={"-".join(addr)}&workers=0#{quote(remarks)}'
